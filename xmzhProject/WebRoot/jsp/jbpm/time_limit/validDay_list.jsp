			...
			<w:panel id="panel" width="100%" title="工作时间配置列表">
              ...
              <td colspan="23" class="command_sort_area">
							...
							<l:greaterThan property="page.count" targetValue="0" compareType="number" >
								&nbsp; &nbsp;
								<input type="button" class="button" value="新增工作时间" onclick="addWorkTime();" />
								<input type="button" class="button" value="新增工作日" onclick="addWorkDay();" />
								...
								<input type="button" class="button" value="查看工作日" onclick="queryWorkDay();" />
							</l:greaterThan>
							</div>
							<div class="h4">
			...
			var gop = $id("group1");
	  		...
	  		if(len == 0){
				...
				strUrl += "workTimeMainBean.startDate="+startDate+"&workTimeMainBean.endDate="+endDate;
	  			showModalCenter(strUrl, null, callBackFunc, 800, 400, '新增工作时间');  
	  		}
		}
		...
		function addWorkDay(){
			var gop = $id("group1");
	  		...
	  		if(len == 0){
	  			...
	  			var row=gop.getSelectRow();
    			var id = row.getParam("id");
				var strUrl = "/jbpm/timeLimitManageAction_toAddWorkDay.action?workTimeMainBean.id="+id;
	  			showModalCenter(strUrl, null, callBackFunc, 800, 400, '新增工作日');  
	  		}
		}
